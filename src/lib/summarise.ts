...
export async function generateSummary(
  ...
  const userMessage = isInfographic
    ? `This is an infographic titled "${title}". Please analyse the image and create a search-optimised summary describing when and how a financial adviser would use this with clients. Write the description in an evergreen fashion — do not mention specific calendar years, dates, or numbers/statistics from the visual. Focus on the concept and the behavioural message, not the specific data points. The description should remain relevant even if the visual is later updated with newer data.`
    : isPdf
  ...
  const response = await getAnthropic().messages.create({
    model: isInfographic ? "claude-opus-4-6" : "claude-sonnet-4-6",
    max_tokens: 500,
    temperature: 0,
